...
class Environment(object):
    ...
    def step(self, target):
        if self.unique_idx == self.n:
            ...
            observation = None
            reward = r_score(self.y_test_full[:len(self.y_pred_full)], self.y_pred_full)
            info = {'public_score': reward}
        else:
            self.y_pred_full += list(target['y'].values)
            reward = [r_score(self.temp_test_y, target['y'].values), r_score(self.y_test_full[:len(self.y_pred_full)], self.y_pred_full)]
            done = False
            info = {}
            timesplit = self.unique_timestamp[self.unique_idx]
            self.unique_idx += 1
